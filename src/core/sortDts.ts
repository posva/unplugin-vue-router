...
/**
 * Determine a deterministic, locale-independent ordering between two route/file paths.
 *
 * Compares paths by normalizing to NFC, stripping leading slashes, and comparing segments left-to-right.
 * Within the same directory, `index.vue` is ordered first and files (segments containing a dot at the last segment)
 * are ordered before folders. Segments are compared using a fixed English collator to ensure stable, locale-insensitive ordering.
 *
